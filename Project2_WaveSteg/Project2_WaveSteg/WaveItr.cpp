...
WaveItr::WaveItr(void) : header(0), headSize(36){}//size of char array to read header
...
bool WaveItr::readHeader(){
	...
	find_iatOneSec();
	cout << finName << dec << ":  iatOneSec=" << iatOneSec << endl;
...
void WaveItr::dispWaveForm(int limit){
	...
	if(interval%2 != 0){interval++;}//make sure interval is an even number
	seekDataStart();
	cout << "Signed max=" << signedMax << ", Interval=" << interval << ", Stream Length=" << streamLen << ", iData=" << iData << ", SampleSize=" << sampleSize << endl;
	while(nextSample(n, interval) && 0 < limit--){
		W.single(n);
	}
	cout << endl;
...
int WaveItr::numZeroes(bool verbose){//locates and counts wave form's zero-crossings
	...
	while(nextSample(n)){
		if(oppSigns(n, last)){
			zeroes++;
			if(verbose){
				cout << "Zero near " << (int)fin.tellg() - iData-1 << endl;
			}
		}
		last = n;
	}
...
char* WaveItr::getHeaderFixed(){//Returns a pointer to a fixed copy of header array
	char* headerCopy = new char[headSize];
	//cout << "orig" << endl;
	for(int i=0;i<headSize;i++){
		//cout << dec << i << "\t" << hex << chint(header[i]) << endl;
		headerCopy[i] = header[i];
	}
	unsigned int fSizeMinus8 = 44 + streamLen + 72;//why 72? idk.  Unaccounted data
	//cout << "fSizeMinus8=" << dec << fSizeMinus8 << endl;
	toCharArray(fSizeMinus8, headerCopy, 4, 4);//fix file size
	toCharArray(16, headerCopy, 16, 4);//fix length of format data to 16
	//cout << "fixed" << endl;
	//for(int i=0;i<headSize;i++){
	//	cout << dec << i << "\t" << hex << chint(headerCopy[i]) << endl;
	//}
	return headerCopy;
}
...
//void WaveItr::dispByte(char* displayMe, int size){//Private, for displaying byte-size data
//	for(int i=0;i<size;i++){
//		int n = displayMe[i];
//		cout << i << "\t" << hex << 0xFF & n ;//in hex
//		cout << "\t" << displayMe[i] << endl;//Char representation, if meaningful
//	}
//}